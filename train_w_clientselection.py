...
def parse_args():
    ...
    parser.add_argument('--user', type=str, default='gsli_extraExps')
    
    parser.add_argument('--agg_mth', type=str, default="FA",
                        choices=['FA', 'kmeans_selection', 'client-client', 'client-server', 'ligengwk', 'ligengwok', 'kmeans'])
    ...
    parser.add_argument('--num_selected', type=int, default=20)
...
def train(args):
    ...
    if args.agg_mth == 'ligengwk' or args.agg_mth == 'ligengwok' or args.agg_mth == 'kmeans':
        train_loader, val_loader, test_loader, class_distribution, client_labels, validation_data_sizes = get_data_loaders_val(args.num_clients, args.batch_size, 
                                                                                                                                ...
                                                                                                                                args.n_iid, args.verbose)
    elif args.agg_mth == 'kmeans_selection':
        train_loader, val_loader, test_loader, number_selected, class_distribution, client_labels, validation_data_sizes = get_data_loaders_kmeans_selection(args.num_clients, args.batch_size, 
                                                                                                                                args.num_clusters, args.classes_pc, 
                                                                                                                                args.real_world, args.validation_split,
                                                                                                                                args.n_iid, args.verbose)
        print("number_selected: ", number_selected)
        client_models = [ VGG('VGG19').to(device) for _ in range(number_selected)]
        for model in client_models:
            model.load_state_dict(global_model.state_dict())

        opt = [optim.SGD(model.parameters(), lr=args.lr) for model in client_models]

    elif args.agg_mth == 'FA':
        train_loader, test_loader = get_data_loaders_FA(classes_pc=args.classes_pc, nclients= args.num_clients, 
                                                batch_size=args.batch_size, real_wd=args.real_world, n_iid=args.n_iid, verbose=args.verbose)
    else:
        raise ValueError("Unsupported agg_mth!")
    ...
    for r in range(args.num_rounds):        
        if args.agg_mth == "kmeans_selection":
            client_lens = None
        else:
        ...
        if args.agg_mth == "kmeans_selection":
            for i in tqdm(range(number_selected)):
                ...
                loss_train += client_update(client_models[i], opt[i], train_loader[i], args.epochs)
        else: 
