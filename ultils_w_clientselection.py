...
def split_balanced_non_iid_w_division(data, labels, n_clients, n_iid, skewness=0.5, max_classes_per_client=2):
    data_per_client = [len(data) // n_clients] * n_clients
    data_per_client[-1] += len(data) - sum(data_per_client)  # 处理不能整除的情况

    clients_split = []
    iid_indices = []  # 收集IID客户端的索引
    non_iid_indices = []  # 收集non-IID客户端的索引
    ...
    for i in range(n_iid):
        ...
        clients_split.append((data[client_indices], labels[client_indices]))
        start = end
        iid_indices.append(i)  # 记录IID客户端索引
    ...
    for i in range(n_iid, n_clients):
        ...
        clients_split.append((data[client_indices], labels[client_indices]))
        non_iid_indices.append(i)  # 记录non-IID客户端索引

    return clients_split, iid_indices, non_iid_indices
...
def get_data_loaders_FA(nclients, batch_size, classes_pc, real_wd, n_iid, verbose=True):
    ...
    if real_wd == 0:
        ...
        # 自定义iid数量
        split = split_balanced_non_iid(x_train, y_train, n_clients=nclients, n_iid=n_iid, max_classes_per_client=classes_pc)
    else:  
        # 最极端
...
def get_data_loaders_val(nclients, batch_size, k, classes_pc=10, real_wd=False, validation_split=0.2, n_iid=2, verbose=True):
...
def get_data_loaders_kmeans_selection(nclients, batch_size, k, classes_pc=10, real_wd=False, validation_split=0.2, n_iid=2, verbose=True):
    x_train, y_train, x_test, y_test = get_cifar10()
    ...
    # Store the client category distribution matrix
    class_distribution_matrix = create_class_distribution_matrix(split)
    class_distribution_matrix.to_csv('class_distribution_matrix_kmeans_selection.csv')
    
    # 应用K-means聚类
    kmeans = KMeans(n_clusters=k, random_state=0).fit(class_distribution)
    client_labels = kmeans.labels_

    # 请在这里增加根据client_labels挑选出iid client，从而跟新split的代码。
    label_counts = {}
    ...
    most_common_label = max(label_counts, key=label_counts.get)  # 通过比较获取最大计数的标签
    print("most_common_label: ", most_common_label)
    print("most_common_label key corresponding value: ", label_counts[most_common_label])
    print("raw split len: ", len(split))
    split = [split[i] for i in range(len(split)) if client_labels[i] == most_common_label]
    print("divided split len: ", len(split))
    ...
    # 返回客户端数据加载器、测试数据加载器、类别分布和客户端标签
    print("client_loaders len: ", len(client_loaders))
    print("val_loaders len: ", len(val_loaders))
    number_selected = label_counts[most_common_label]
    
    return client_loaders, val_loaders, test_loader, number_selected, class_distribution, client_labels, validation_data_sizes
...
def server_aggregate_ligeng_wok(global_model, client_models, client_data_loaders, validation_data_sizes, client_labels, client_indices, k):
    ...
    client_accuracies = [evaluate_model_on_multiple_loaders(client_models[i], 
                                                            [client_data_loaders[j] for j in range(len(client_models))], 
                                                            [validation_data_sizes[j] for j in range(len(client_models))]) for i in range(len(client_models))]
...
def server_aggregate_kmeans_selection(global_model, client_models, client_labels, client_indices, k, num_clients):
    client_indices = list(client_indices) if isinstance(client_indices, np.ndarray) else client_indices
    
    
    cluster_counts = [sum(client_labels == i) for i in range(k)]
    cluster_weights = {i: count / sum(cluster_counts) for i, count in enumerate(cluster_counts)}
