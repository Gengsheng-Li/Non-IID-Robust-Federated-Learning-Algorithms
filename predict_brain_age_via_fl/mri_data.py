...
def generate_training_dataloders(data_dir, num_clients, batch_size, name_experiment):
    train_dataloaders = []

    for client in range(1, num_clients + 1):
        train_dataset = OpenBHBDataset(data_dir + '/train_quasiraw',
                                       f'tsv/{name_experiment}/site_{client}.tsv',
                                       modality='quasiraw',
                                       transform=transforms.ToTensor())
        train_dataloaders.append(DataLoader(train_dataset, shuffle=True, batch_size=batch_size, pin_memory=True))

    return train_dataloaders

def generate_testing_dataloder(data_dir, batch_size, name_experiment):
    test_dataloaders = []

    test_dataset = OpenBHBDataset(data_dir + '/val_quasiraw',
                                    f'tsv/{name_experiment}/test.tsv',
                                    modality='quasiraw',
                                    transform=transforms.ToTensor())
    test_dataloaders.append(DataLoader(test_dataset, shuffle=False, batch_size=batch_size, pin_memory=True))
